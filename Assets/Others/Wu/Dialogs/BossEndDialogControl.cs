...
public class BossEndDialogControl : MonoBehaviour
{
    [Header("NPC姓名")]
    ...
    private void showText() //链接txt文本与UI界面Text 并且逐行读取显示 读取完毕隐藏UI
    {
        ...
        if (isTalking && Input.GetKeyDown(KeyCode.E))
        {
            ...
            if(namenow.Equals("八云紫\r")){
                spritnow =  Zi;

            }
            else if(namenow.Equals("小铃\r")){
                spritnow = Ling;
            }
            else if(namenow.Equals("琪露诺\r")){
                spritnow = Qi;
                
            }
            else if(namenow.Equals("阿求\r")){
                spritnow = Qiu;
            }
            else{
                spritnow = null;
        ...
        if (textRow == str.Length)
        {
            ...
            isTalking = false;
            Time.timeScale = 1;
            GameManage.sGameManage.changeScene("TheEnd",1);
